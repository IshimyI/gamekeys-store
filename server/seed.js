...
function seed() {
  const insertProduct = db.prepare(
    'INSERT OR IGNORE INTO products (sku, name, type, price, currency, image) VALUES (?, ?, ?, ?, ?, ?)'
  );
  for (const p of PRODUCTS) {
    insertProduct.run(p.sku, p.name, p.type, p.price, p.currency, p.image);
  }

  const insertKey = db.prepare('INSERT OR IGNORE INTO keys (code, status) VALUES (?, \'available\')');
  for (const code of KEYS) {
    insertKey.run(code);
  }

  const insertPromo = db.prepare(
    'INSERT OR IGNORE INTO promo_codes (code, type, value, currency, max_uses, used_count) VALUES (?, ?, ?, ?, ?, 0)'
  );
  for (const p of PROMO_CODES) {
    insertPromo.run(p.code, p.type, p.value, p.currency, p.max_uses);
  }
  ...
  console.log(`Seed OK: products=${productCount}, keys=${keyCount}, promo_codes=${promoCount}`);
}
