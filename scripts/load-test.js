...
async function createOrder(sku, opts = {}) {
  const res = await fetch(`${BASE}/api/orders`, {
    ...
    body: JSON.stringify({ sku, promo_code: opts.promoCode, id: opts.id }),
  });
...
async function scenarioDuplicateEventIdNoOp() {
  ...
  const first = await sendWebhook('dup-evt', order.order_id);
...
async function scenarioEmptyPoolAndRestock() {
  console.log('\n[5] Пустой пул ключей → out_of_stock, без падения; после пополнения повторная выдача даёт ровно один ключ');
...
async function scenarioPromoLimitUnderConcurrency() {
  console.log('\n[6] Промокод с лимитом N под параллельными запросами применён не более N раз (LIMIT3, max_uses=3)');
  const attempts = 20;
  const results = await Promise.all(
    Array.from({ length: attempts }, () => createOrder('SUB-SPOTIFY-1M', { promoCode: 'LIMIT3' }))
  );
  const applied = results.filter((r) => r.data.promo_code === 'LIMIT3').length;
  const rejected = results.filter((r) => r.data.error === 'promo_limit_reached').length;
  assert(applied === 3, `промокод применён ровно 3 раза из ${attempts} параллельных попыток (получили ${applied})`);
  assert(rejected === attempts - 3, `остальные ${attempts - 3} получили promo_limit_reached (получили ${rejected})`);
}
...
async function main() {
  ...
  await scenarioPromoLimitUnderConcurrency();
